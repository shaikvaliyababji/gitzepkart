...
// System.out.println("✅ JWT validated. User authenticated: " + email + ", Role:
// " + role);
// }
// } else {
...
public class JWTAuthFilter extends OncePerRequestFilter {
    ...
    // Public endpoints (supports wildcard using startsWith)
    private static final List<String> PUBLIC_PATHS = List.of(
            "/api/users/register",
            "/api/users/login",
            "/api/users/signin",
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        System.out.println("Requested path: " + method + " " + path);

        // 1️⃣ Allow CORS preflight (MUST HAVE)
        if ("OPTIONS".equalsIgnoreCase(method)) {
            System.out.println("🟡 OPTIONS preflight — skipping JWT filter");
            response.setStatus(HttpServletResponse.SC_OK);
            filterChain.doFilter(request, response);
            return;
        }

        // 2️⃣ Allow any public endpoints
        if (isPublicPath(path)) {
            System.out.println("🟢 Public endpoint — skipping JWT: " + path);
            filterChain.doFilter(request, response);
            return;
        }

        // 3️⃣ Normal secured request with JWT
        String authHeader = request.getHeader("Authorization");
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                System.out.println("🟢 JWT Validated: " + email + " | Role: " + role);
            }
        } else {
    ...
    // Wildcard support for public URLs
